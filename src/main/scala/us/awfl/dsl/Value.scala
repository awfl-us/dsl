...
def encodeJson(value: Resolved[_]): Cel = CelFunc("json.encode_to_string", value.cel)
...
case class Value[T: Spec](resolver: Resolver) extends Resolved[T] {
  ...
  def flatMap[U](f: T => ListValue[U]): ListValue[U] = f(get)
  def flatMap[U: Spec](f: T => BaseValue[U]): Value[U] = f(get) match {
    case value: Value[U] => value
    case _: Obj[U] => sys.error("Unexpected Obj in Value flatMap")
    case ListValue(r) => Value(r)
  }
...
sealed trait OptBase[T] {
  def getOrElse(default: Value[T]): Value[T]
  def map[U: Spec](f: Value[T] => Value[U]): OptBase[U]
}
case class OptResolved[T: Spec](resolver: Resolver) extends OptBase[T] {
  def getOrElse(default: Value[T]): Value[T] = resolver.path.path match {
  ...
  def map[U: Spec](f: Value[T] => Value[U]): OptBase[U] = OptResolved(f(Value[T](resolver)).resolver)
...
object OptValue {
  ...
  def apply[T: Spec](obj: Obj[T]): OptBase[T] = OptObj(obj)
  def apply[T: Spec](value: Value[T]): OptValue[T] = OptResolved[T](value.resolver)
  def nil[T: Spec]: OptValue[T] = OptValue(CelConst("null"))
}

case class OptList[T: Spec](resolver: Resolver) {
  def getOrElse(default: ListValue[T]): ListValue[T] = ListValue(OptResolved(resolver).getOrElse(Value(default.cel)).cel)
